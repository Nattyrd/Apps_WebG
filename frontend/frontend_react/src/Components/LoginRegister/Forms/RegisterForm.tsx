...
const Container = styled.div`
  background-color: ${({ theme }) => (theme as AppTheme).body};
  padding: 40px;
  animation: ${fadeIn} 1s;
`
...
const Input = styled.input`
  ...
  padding-inline: 8px;
  margin-bottom: 15px;
  color: ${({ theme }) => (theme as AppTheme).text};
  outline: 1px solid ${({ theme }) => (theme as AppTheme).border};
  border: none;
...
const RegisterForm: React.FC = () => {
  ...
  const handleRegister = async () => {
    ...
    try{
      ...
      let res = await axios.post(process.env.REACT_APP_API_URL + '/users/register', register, {headers});
